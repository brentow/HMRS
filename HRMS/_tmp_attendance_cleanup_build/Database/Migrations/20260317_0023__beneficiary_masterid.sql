...
SET @MASTERID_EXISTS = (
		...
		WHERE TABLE_SCHEMA = DATABASE()
			...
			AND COLUMN_NAME = 'MasterID'
);

SET @ADD_MASTERID_SQL = IF(
		@USER_ID_COLUMN_TYPE IS NULL OR @MASTERID_EXISTS > 0,
		'SELECT 1',
		CONCAT('ALTER TABLE BeneficiaryStaging ADD COLUMN MasterID ', @USER_ID_COLUMN_TYPE, ' NULL')
);
